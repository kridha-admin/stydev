...
export function resolveFabricProperties(garment) {
    ...
    // Base cling risk
    const gsmFactor = Math.max(0, 1.0 - effectiveGsm / 300.0);
    const frictionFactor = Math.max(0, 1.0 - garment.surface_friction);
    const clingBase = Math.min(1.0, (totalStretch / 20.0 + gsmFactor + frictionFactor) / 3.0);
