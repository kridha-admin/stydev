...
async function extractWithModel(imageBase64, modelId, systemPrompt, userPrompt, modelLabel) {
    ...
    if (!responseText) {
        throw new Error(`No response from ${modelLabel}`);
    }

    const attributes = parseJsonResponse(responseText);
...
async function extractTieredParallelGemini(imageSource) {
    ...
    const [flashLiteResult, flashResult] = await Promise.all([
        extractWithModel(imageBase64, MODEL_FLASH_LITE, SYSTEM_PROMPT, USER_PROMPT_FLASH_LITE, 'Flash-Lite'),
        extractWithModel(imageBase64, MODEL_FLASH, SYSTEM_PROMPT, USER_PROMPT_FLASH, 'Flash')
    ]);
 ...
 */
export async function extractGarmentAttributesGeminiTiered(imageSource) {
...
async function main() {
    ...
    for (let i = 0; i < samplecount; i++) {
        ...
        const result = await extractGarmentAttributesGeminiTiered(testImage);
        if (result.success) {
