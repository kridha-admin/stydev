...
const PRINCIPLE_PHRASINGS = {
    hemline: {
        ...
        mild_negative: {
            _default: [
                "the length is close but might land an inch or two off your best spot",
            ],
        },
        moderate_negative: {
            _default: [
                "the hem length isn't ideal for your height — it may cut your leg line short",
            ],
        },
        fix: [
    ...
    bodycon_mapping: {
        ...
        mild_negative: {
            _default: [
                "the fabric might show more than you want — worth checking the weight in person",
                "the stretch could follow your shape a bit closely — try it on first",
            ],
        },
        moderate_negative: {
            _default: [
                "the fabric is on the thin side — it may cling rather than skim",
                "the fabric doesn't have quite enough structure to hold a smooth line",
            ],
            pear: [
                "the fabric may follow through the hip more than you'd like",
            ],
            apple: [
                "the fabric weight might not be enough to smooth through the midsection",
            ],
        },
        fix: [
    ...
    dark_black_slimming: {
        ...
        mild_negative: {
            _default: [
                "the color is on the lighter side — a darker shade would slim a touch more",
            ],
        },
        moderate_negative: {
            _default: [
                "the light color visually expands — a darker tone would create a sleeker line",
            ],
        },
        fix: [
    ...
    tent_concealment: {
        ...
        mild_negative: {
            _default: [
                "the silhouette is a little loose — adding a belt could give it more shape",
            ],
        },
        moderate_negative: {
            _default: [
                "the shape is quite loose — it may lose your waist definition",
            ],
            hourglass: [
                "the loose shape hides the waist definition you're going for",
            ],
        },
        fix: [
    ...
    fabric_zone: {
        ...
        mild_negative: {
            _default: [
                "the fabric weight is borderline — check it in person to see if it holds its shape",
                "worth feeling the fabric first — it might be lighter than it looks online",
            ],
        },
        moderate_negative: {
            _default: [
                "the fabric could use more structure — it's on the light side for your frame",
                "the fabric weight may not be enough to create a clean silhouette",
            ],
        },
        fix: [
    ...
    matte_zone: {
        ...
        mild_negative: {
            _default: [
                "there's a slight sheen — it may catch light in a way that adds a bit of width",
            ],
        },
        moderate_negative: {
            _default: [
                "the fabric has noticeable shine — it'll catch light and expand the silhouette",
            ],
        },
        fix: [
...
/**
 * Select a phrase with severity gradation based on principle score.
 * Strong negative (< -0.20): use "negative" bank (alarming language)
 * Moderate negative (-0.20 to -0.08): use "moderate_negative" if available, else "negative"
 * Mild negative (-0.08 to -0.05): use "mild_negative" if available, else skip
 * Near-zero (> -0.05): no negative phrase at all
 */
function phraseWithSeverity(principleName, score, bodyShape, seed, hemZone = "") {
    const absScore = score || 0;
    if (absScore < -0.20) {
        // Strong negative — use full negative language
        return phrase(principleName, "negative", bodyShape, seed, hemZone);
    } else if (absScore < -0.08) {
        // Moderate — try moderate_negative, fall back to negative
        const mod = phrase(principleName, "moderate_negative", bodyShape, seed, hemZone);
        return mod || phrase(principleName, "negative", bodyShape, seed, hemZone);
    } else if (absScore < -0.05) {
        // Mild — try mild_negative, don't fall back to alarming language
        return phrase(principleName, "mild_negative", bodyShape, seed, hemZone);
    }
    // Too close to zero — suppress
    return null;
}
...
export function generateGoldOutput(scored) {
    ...
    const negThreshold = verdict === "smart_pick" ? -0.05 : -0.05;
...
function pinchSP(bodyShape, gWord, positives, negatives, hemZone, sid) {
    ...
    // Negative — use severity-aware phrase selection
    if (negatives.length > 0) {
        const negScore = negatives[0].score || 0;
        const n = phraseWithSeverity(negatives[0].name, negScore, bodyShape, sid + "n1", hemZone);
        if (n) {
            // Mild negatives get softer connectors
            const connector = negScore > -0.08 ? "Just note: " : "But ";
            const style = negScore > -0.08 ? "caution" : "negative";
            segs.push({ text: connector, style: "normal" });
            segs.push({ text: n + ". ", style });
    ...
    // Fallback when negatives have no phrasing or are too mild
    if (negatives.length > 0 && (negatives[0].score || 0) < -0.08) {
        // Moderate/strong negative with no phrase match — use generic
        ...
        // No negatives, or all negatives are mild — give practical try-on advice
