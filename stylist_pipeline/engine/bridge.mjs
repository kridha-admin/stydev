...
const BODY_INTERACTION_MAP = {
    // <clinging|skimming|standing_away|draping_away|null>
    clinging: -0.03,
    skimming: 0.0,
    standing_away: 0.05,
    draping_away: 0.03,
...
function normalizeFiber(fiberName) {
    if (fiberName == null) return null;
    if (!fiberName) return null;
...
function detectCovers(category, hemPosition) {
    let coversWaist = null;
    let coversHips = null;

    // Categories that cover waist
    const waistCoveringCategories = [
        GarmentCategory.DRESS, GarmentCategory.TOP, GarmentCategory.BOTTOM_PANTS, GarmentCategory.BOTTOM_SHORTS, GarmentCategory.SKIRT,
        GarmentCategory.JUMPSUIT, GarmentCategory.ROMPER, GarmentCategory.JACKET, GarmentCategory.COAT, GarmentCategory.SWEATSHIRT,
        GarmentCategory.CARDIGAN, GarmentCategory.VEST, GarmentCategory.BODYSUIT, GarmentCategory.LOUNGEWEAR, GarmentCategory.ACTIVEWEAR,
        GarmentCategory.SAREE, GarmentCategory.SALWAR_KAMEEZ, GarmentCategory.LEHENGA,
    ];

    // Categories that inherently don't cover hips (regardless of hem)
    const nonHipCoveringCategories = [
        GarmentCategory.TOP, GarmentCategory.VEST,
    ];

    // Categories that inherently cover hips (regardless of hem)
    const hipCoveringCategories = [
        GarmentCategory.BOTTOM_PANTS, GarmentCategory.BOTTOM_SHORTS,  GarmentCategory.JUMPSUIT,
        GarmentCategory.ROMPER,  GarmentCategory.BODYSUIT, GarmentCategory.SAREE,
        GarmentCategory.SALWAR_KAMEEZ,  GarmentCategory.LEHENGA,
    ];

    // Hem-based hip coverage (using mapped values)
    const hipCoveringHems = ["above_knee", "knee", "below_knee", "midi", "below_calf", "ankle", "floor"];
    const nonHipCoveringHems = ["mini"];

    // Determine waist coverage
    if (category != null && waistCoveringCategories.includes(category)) {
        coversWaist = true;
    }

    // Determine hip coverage
    if (category != null && hipCoveringCategories.includes(category)) {
        coversHips = true;
    } else if (category != null && nonHipCoveringCategories.includes(category)) {
        coversHips = false;
    } else if (hemPosition != null) {
        // For dresses, skirts, coats, etc. - depends on hem
        if (hipCoveringHems.includes(hemPosition)) {
            coversHips = true;
        } else if (nonHipCoveringHems.includes(hemPosition)) {
            coversHips = false;
        }
    }
...
export function resolveFabricGsm(garmentAttrs) {
    ...
    const stretchPct = safeGet(garmentAttrs, "stretch_percentage");
...
/**
 * Derive styling goals from body measurements (Tier 2).
 * These are inferred from proportions, not explicitly chosen by the user.
 * All weights are 0.5 (below user-selected 1.0, above universal 0.25).
 *
 * @param {Object} u - Raw user measurements (in cm)
 * @returns {Array<{goal: string, weight: number}>}
 */
function deriveBodyGoals(u) {
    const derived = [];
    const waist = u.waist_circumference || 0;
    const hip = u.hip_circumference || 0;
    const bust = u.chest_circumference || 0;
    const height = u.height || 167.64;
    const heightInches = height * CM_TO_IN;
    const whr = hip > 0 ? waist / hip : 0.80;

    // Wide waist relative to hips → hide midsection
    if (whr > 0.85) {
        derived.push({ goal: "hide_midsection", weight: 0.5 });
    }

    // Hips significantly larger than bust → minimize hips
    if (hip - bust > 3 * 2.54) {
        derived.push({ goal: "minimize_hips", weight: 0.5 });
    }

    // Bust significantly larger than hips → balance shoulders
    if (bust - hip > 3 * 2.54) {
        derived.push({ goal: "balance_shoulders", weight: 0.5 });
    }

    // Petite → look taller
    if (heightInches < 63) {
        derived.push({ goal: "look_taller", weight: 0.5 });
    }

    // Tall → elongate legs (lower priority)
    if (heightInches > 69) {
        derived.push({ goal: "elongate_legs", weight: 0.25 });
    }

    // Plus size → streamline silhouette
    // Use pre-computed size_category when available, fall back to measurement thresholds
    const isPlusSize = u.size_category === "plus_size" || bust * CM_TO_IN > 42 || hip * CM_TO_IN > 44;
    if (isPlusSize) {
        derived.push({ goal: "streamline_silhouette", weight: 0.5 });
    }
...
export function buildBodyProfile(userMeasurements, stylingGoals = null) {
    ...
    const height = safeGet(u, "height") ? safeGet(u, "height") * CM_TO_IN : null;
    const bust = safeGet(u, "chest_circumference") ? safeGet(u, "chest_circumference") * CM_TO_IN : null;
    const waist = safeGet(u, "waist_circumference") ? safeGet(u, "waist_circumference") * CM_TO_IN : null;
    const hip = safeGet(u, "hip_circumference") ? safeGet(u, "hip_circumference") * CM_TO_IN : null;
    const shoulderWidth = safeGet(u, "shoulder_breadth") ? safeGet(u, "shoulder_breadth") * CM_TO_IN : null;
    const neckCircumference = safeGet(u, "neck_circumference") ? safeGet(u, "neck_circumference") * CM_TO_IN : null;
    const armLength = safeGet(u, "arm_right_length") ? safeGet(u, "arm_right_length") * CM_TO_IN : null;
    const inseam = safeGet(u, "inside_leg_height") ? safeGet(u, "inside_leg_height") * CM_TO_IN : null;
    const cThighMax = safeGet(u, "thigh_left_circumference") ? safeGet(u, "thigh_left_circumference") * CM_TO_IN : null;
    const cAnkle = safeGet(u, "ankle_left_circumference") ? safeGet(u, "ankle_left_circumference") * CM_TO_IN : null;
    ...
    const hKnee = safeGet(u, "knee_from_floor") ? safeGet(u, "knee_from_floor") : null;
    const hCalfMax = safeGet(u, "widest_calf_from_floor") ? safeGet(u, "widest_calf_from_floor") : null;
    const hCalfMin = safeGet(u, "mid_calf_from_floor") ? safeGet(u, "mid_calf_from_floor") : null;
    const hAnkle = safeGet(u, "ankle_from_floor") ? safeGet(u, "ankle_from_floor") : null;
    const torsoLength = safeGet(u, "natural_waist_from_shoulder") ? safeGet(u, "natural_waist_from_shoulder") : null;
    const legLengthVisual = safeGet(u, "natural_waist_from_floor") ? safeGet(u, "natural_waist_from_floor") : null;

    // Estimated fields
    // Underbust = ribcage circumference ≈ slightly larger than waist.
    // Old code: bust - 4.0 (everyone gets D-cup bust_differential).
    // New: estimate from waist. Ribcage is typically ~7% wider than waist.
    // Then bust_differential = bust - underbust = actual bust projection.
    // Examples:
    //   Hourglass (bust=36, waist=27): underbust=28.9 → bust_diff=7.1 (DD/DDD) ✓
    //   Rectangle (bust=34, waist=30): underbust=32.1 → bust_diff=1.9 (A/B) ✓
    //   Plus Apple (bust=44, waist=40): underbust=42.8 → bust_diff=1.2 (minimal projection) ✓
    //   Petite Pear (bust=34, waist=27): underbust=28.9 → bust_diff=5.1 (D) ✓
    let underbust;
    if (bust && waist) {
        // Ribcage ≈ waist * 1.07, clamped to physical bounds
        underbust = Math.max(waist, Math.min(bust - 1, waist * 1.07));
    } else if (bust) {
        underbust = bust - 4.0; // fallback to old behavior when waist missing
    } else {
        underbust = null;
    }
    const sizeCategory = safeGet(u, "size_category") ? safeGet(u, "size_category") : null;
    const cUpperArmMax = sizeCategory ? (sizeCategory === "plus_size" ? 14.0 : 11.0) : null;
    const cCalfMax = cAnkle ? cAnkle * 1.6 : null;

    // ── Three-Tier Goal Weighting System ──
    // Tier 1: User-selected goals (weight 1.0)
    // Tier 2: Body-derived goals (weight 0.5)
    // Tier 3: Universal goals (weight 0.25)
    // Deduplicated: highest weight wins per engine goal.

    const goalWeightMap = new Map(); // StylingGoal enum -> weight

    // Tier 1: Map user-selected goals
    const goalsSource = stylingGoals || u.styling_goals || [];
    for (const g of goalsSource) {
        if (typeof g === 'object' && g.goal) {
            // Weighted goal object: { goal: "look_taller", weight: 1.0 }
            const mapped = GOAL_MAP[g.goal];
            if (mapped) {
                const existing = goalWeightMap.get(mapped) || 0;
                goalWeightMap.set(mapped, Math.max(existing, g.weight));
            }
        } else {
            // Plain string (backward compat) — weight 1.0
            const mapped = GOAL_MAP[g];
            if (mapped) {
                const existing = goalWeightMap.get(mapped) || 0;
                goalWeightMap.set(mapped, Math.max(existing, 1.0));
            }
        }
    }

    // Tier 2: Body-derived goals (only if not already present from Tier 1)
    const bodyDerived = deriveBodyGoals(u);
    for (const d of bodyDerived) {
        const mapped = GOAL_MAP[d.goal];
        if (mapped && !goalWeightMap.has(mapped)) {
            goalWeightMap.set(mapped, d.weight);
        }
    }

    // Tier 3: Universal goals (only if not already present from Tier 1 or 2)
    const universalGoals = [
        { goal: "highlight_waist", weight: 0.25 },
        { goal: "look_proportional", weight: 0.25 },
    ];
    for (const ug of universalGoals) {
        const mapped = GOAL_MAP[ug.goal];
        if (mapped && !goalWeightMap.has(mapped)) {
            goalWeightMap.set(mapped, ug.weight);
        }
    }

    // Build final deduplicated weighted goals array
    const goals = Array.from(goalWeightMap.entries()).map(
    ...
    // ── Derive per-zone goals and concern levels from styling_goals ──
    // These activate dead code paths in hemline, neckline, waist, and belly scorers.
    // Without this mapping, goal_bust/goal_legs/goal_hip/belly_zone are always null/0.
    let goalBust = null;
    let goalLegs = null;
    let goalHip = null;
    let goalArm = null;
    ...
    for (const entry of goals) {
        const g = entry.goal;
        if (g === StylingGoal.HIDE_MIDSECTION) {
            goalBelly = "minimize";
            bellyZone = 0.7; // Activates belly-specific logic in tent/bodycon/fabric scorers
        } else if (g === StylingGoal.SLIM_HIPS) {
            goalHip = "minimize";
        } else if (g === StylingGoal.LOOK_TALLER) {
            goalLegs = "elongate";
        } else if (g === StylingGoal.MINIMIZE_ARMS) {
            goalArm = "minimize";
        } else if (g === StylingGoal.EMPHASIS) {
            goalBust = "emphasize";
        } else if (g === StylingGoal.CONCEALMENT) {
            goalBust = "minimize";
        }
    }
    ...
    return new BodyProfile({
        ...
        belly_zone: bellyZone,
        goal_bust: goalBust,
        goal_legs: goalLegs,
        goal_hip: goalHip,
        goal_arm: goalArm,
        goal_belly: goalBelly,
    });
}
...
export function buildGarmentProfile(garmentAttrs) {
    ...
    // console.log("garmentAttrs: ", garmentAttrs);
    ...
    if (necklineDepthStr) {
        ...
        vDepthCm = safeGet(depthMap, necklineDepthStr, null);
    }
    ...
    let primaryFiber = safeGet(g, "fabric_primary") ? normalizeFiber(safeGet(g, "fabric_primary").toLowerCase()) : null;
    let secondaryFiber = safeGet(g, "fabric_secondary") ? normalizeFiber(safeGet(g, "fabric_secondary").toLowerCase()) : null;

    // GSM resolution
    const fabricResolution = resolveFabricGsm(g);
    const gsmEstimated = fabricResolution.gsm;
    const gsmConfidence = fabricResolution.confidence || "moderate";

    // const drape = DRAPE_MAP[safeGet(g, "fabric_drape")] || 5.0;
    const drape = safeGet(DRAPE_MAP, safeGet(g, "fabric_drape"), null);

    // Construction
    // let construction = FIBER_CONSTRUCTION_MAP[primaryFiber] || FabricConstruction.WOVEN;
    let construction = safeGet(FIBER_CONSTRUCTION_MAP, primaryFiber, null);
    ...
    if (bodyInteraction) {
        ...
        const interactionAdj = safeGet(BODY_INTERACTION_MAP, bodyInteraction);
        expansionRate = (expansionRate != null || interactionAdj != null)
                        ? Math.max(0.0, (expansionRate ?? 0) + (interactionAdj ?? 0))
                        : null;
    }

    // Stretch visible
    const stretchVisible = safeGet(g, "fabric_stretch_visible");
    if (stretchVisible != null && stretchVisible === true && elastanePct != null && elastanePct < 3) {
        elastanePct = Math.max(elastanePct, 3.0);
    }

    // Hemline
    // const hemPosition = HEM_POSITION_MAP[safeGet(g, "hemline_position")] || "knee";
    const hemPosition = safeGet(HEM_POSITION_MAP, safeGet(g, "hemline_position"));

    // Waist
    // const waistPosition = WAIST_POSITION_MAP[safeGet(g, "waistline")] || "natural";
    const waistPosition = safeGet(WAIST_POSITION_MAP, safeGet(g, "waistline"));
    ...
    const colorLightness = safeGet(COLOR_LIGHTNESS_MAP, safeGet(g, "color_value"));
    const colorSaturation = safeGet(COLOR_SATURATION_MAP, safeGet(g, "color_saturation"));
    const colorTemperature = safeGet(g, "color_temperature");
    ...
    let patternScale = safeGet(g, "pattern_scale");
    if (!hasPattern && patternScale != null) patternScale = "none";
    const patternContrast = safeGet(PATTERN_CONTRAST_MAP, safeGet(g, "pattern_contrast"));

    // Construction details
    // NOTE: has_seaming REMOVED from isStructured — Bedrock returns true for almost
    // every garment (all garments have visible seams), which made GATE_STRUCTURED fire
    // for everything → 70% penalty reduction on ALL negatives. "Structured" means
    // self-supporting construction (darts + stiff/structured fabric), not just "has seams".
    ...
    const modelHeightInches = safeGet(g, "model_height_inches", 69);
...
if (import.meta.url === `file://${process.argv[1]}`) {
    let garmentAttrs = {
        ...
        fabric_composition: 'Short jacket in cotton denim with a deep V-neckline, two sets of narrow ties at the...',
        ...
        garment_type: 'jacket',
        title: 'Tie-front denim jacket - Denim blue - Ladies | H&M IN',
        brand: null,
        price: null,
    ...
    let garmentProfile = buildGarmentProfile(garmentAttrs);
    console.log(garmentProfile);
}
