...
export class BodyProfile {
    constructor(data = {}) {
        ...
        // Leg measurements — height-scaled defaults
        // Anthropometric ratios (from floor): knee ~27%, widest calf ~21%, narrowest calf ~15%, ankle ~6%
        // Previous fixed defaults (18/14/10/4) only fit a ~66" person. A 5'2" woman (62")
        // and a 5'10" woman (70") need different landmarks for hemline scoring to work.
        const h = this.height; // already set above
        this.h_knee = data.h_knee ?? (h * 0.273);       // 66" -> 18.0, 62" -> 16.9, 70" -> 19.1
        this.h_calf_max = data.h_calf_max ?? (h * 0.212); // 66" -> 14.0, 62" -> 13.1, 70" -> 14.8
        this.h_calf_min = data.h_calf_min ?? (h * 0.152); // 66" -> 10.0, 62" -> 9.4, 70" -> 10.6
        this.h_ankle = data.h_ankle ?? (h * 0.061);      // 66" -> 4.0, 62" -> 3.8, 70" -> 4.3
        this.c_thigh_max = data.c_thigh_max ?? 22.0;
        this.c_calf_max = data.c_calf_max ?? 14.5;
        this.c_calf_min = data.c_calf_min ?? 11.5;       // Was 9.0 → prominence 1.611 (absurdly high)
        this.c_ankle = data.c_ankle ?? 8.5;               // Now prominence = 14.5/11.5 = 1.261 (reasonable)
...
export class GarmentProfile {
    constructor(data = {}) {
        ...
        this.gsm_confidence = data.gsm_confidence ?? "moderate";  // high, moderate, low, very_low
        this.surface = data.surface ?? SurfaceFinish.MATTE;
        this.surface_friction = data.surface_friction ?? 0.5;
        this.drape = data.drape ?? null;  // null = unknown drape (0-10 scale)

        // Silhouette
        this.category = data.category ?? GarmentCategory.DRESS;
        this.silhouette = data.silhouette ?? Silhouette.SEMI_FITTED;
        this.expansion_rate = data.expansion_rate ?? null;  // null = unknown fit, scorers must guard
        ...
        // Hemline
        this.hem_position = data.hem_position ?? null;  // null = unknown, hemline scorer skips
        ...
        // Color
        this.color_lightness = data.color_lightness ?? null;  // null = unknown color value
...
export class PrincipleResult {
    constructor(data = {}) {
        ...
        this.prompt_input_reasoning = data.prompt_input_reasoning ?? [];
    }
}
...
export class GoalVerdict {
    constructor(data = {}) {
        ...
        this.supportingReasoning = data.supportingReasoning ?? [];
        this.reasoning = data.reasoning ?? "";
    }
}
...
// ================================================================
// GOAL UTILITIES — works with weighted goals [{goal, weight}]
// ================================================================

/**
 * Check if a goal exists in a styling_goals array.
 * Supports both new format [{goal, weight}] and legacy [string].
 */
export function hasGoal(goals, goal) {
    if (!goals || !Array.isArray(goals)) return false;
...
/**
 * Get the weight of a goal (0 if not present).
 * Supports both new format [{goal, weight}] and legacy [string] (weight=1.0).
 */
export function getGoalWeight(goals, goal) {
    if (!goals || !Array.isArray(goals)) return 0;
    for (const entry of goals) {
        const g = entry.goal || entry;
        if (g === goal) return entry.weight ?? 1.0;
    }
    return 0;
}
...
// Rescale curve v2 — fixes over-generous inflation
// Key mappings (composite → raw10 → display):
//   -0.40 → 3.0 → 1.0  (terrible = deep NTO)
//   -0.10 → 4.5 → 3.5  (bad = clear NTO)
//    0.00 → 5.0 → 4.5  (neutral = borderline NTO — don't recommend neutrals)
//   +0.05 → 5.25 → 5.5 (slightly positive = low SP)
//   +0.10 → 5.5 → 6.5  (positive = mid SP)
//   +0.15 → 5.75 → 7.5 (clearly positive = high SP)
//   +0.20 → 6.0 → 8.5  (strongly positive = TII)
//   +0.30 → 6.5 → 9.0  (exceptional = strong TII)
const RESCALE_BREAKPOINTS = [
    [0.0,  3.0,  0.0,  1.0],   // terrible negatives → 0-1
    [3.0,  4.0,  1.0,  2.5],   // bad → 1-2.5
    [4.0,  4.5,  2.5,  3.5],   // poor → 2.5-3.5
    [4.5,  5.0,  3.5,  4.5],   // neutral-ish → 3.5-4.5 (NTO territory)
    [5.0,  5.25, 4.5,  5.5],   // slightly positive → 4.5-5.5 (low SP)
    [5.25, 5.5,  5.5,  6.5],   // positive → 5.5-6.5 (mid SP)
    [5.5,  5.75, 6.5,  7.5],   // clearly positive → 6.5-7.5 (high SP)
    [5.75, 6.0,  7.5,  8.5],   // strongly positive → 7.5-8.5 (TII threshold)
    [6.0,  6.5,  8.5,  9.0],   // exceptional → 8.5-9.0
    [6.5,  10.0, 9.0,  10.0],  // extraordinary → 9.0-10
];
